...
class Window(Frame):
    def __init__(self, master=None):
        ...
        B2 = Button(F1, text="Kitap Listele", command=self.listele, font="bold", cursor="hand2", fg="green", width=13,
                    height=3)
    ...
    def kayit_et(self):
        try:
            self.im.execute(
                "CREATE TABLE IF NOT EXISTS kitaplar (id INTEGER PRIMARY KEY, Kitap_Adi VARCHAR(45), Yazar VARCHAR(45), Yayin_Evi VARCHAR(45), Sayfa_Sayisi INT, Baski INT, Yayin_Yili)")  # Tablo oluşturma
            self.im.execute(
    ...
    def Duzenle(self):
        ...
        def veriKayit():
            ...
            self.im.execute(
                "CREATE TABLE IF NOT EXISTS kitaplar (Kitap Adı VARCHAR(45), Yazar VARCHAR(45), Yayin Evi VARCHAR(45), Sayfa Sayısı INT, Baskı INT, Yayın Yılı)")  # Tablo oluşturma

            self.im.execute(
                "UPDATE kitaplar SET Kitap_Adi = ?, yazar = ?, Yayin_Evi = ?, Sayfa_Sayisi = ?, Baski = ?, Yayin_Yili = ? WHERE id = ?",
...
me = Label(root, text="Developer: beyzaatay.com | @beyzaatay", fg="#6E7371", cursor="hand2",
           font="Verdana 7 bold")
me.pack(side=BOTTOM)
me.bind("<Button-1>", lambda e: callback(webbrowser.open_new("")))
